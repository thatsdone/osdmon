...
def get_osd_df():
    ...
    if not dry_run and not debug:
        ...
        # NOTE: save result of 'ceph osd df -f json' as 'test-result.json'
...
def check_threshold(data, thr_high, thr_low):
    ...
    osds = sorted(data['nodes'], key=lambda x: x['utilization'], reverse=True)
    for osd in osds:
        #logger.debug("%4d %5.2f %d %d %s %s" % (osd['id'], osd['utilization'], osd['kb'], osd['kb_used'], osd['var'], osd['status']))
...
def set_osd_reweight(osd_id, reweight):
    ...
    if not dry_run:
        ...
        # log result of 'ceph osd reweight' command
        logger.info(buf)
